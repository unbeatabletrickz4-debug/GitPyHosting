...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("👋 **Python & Git Hosting Bot**", reply_markup=main_menu_keyboard(), parse_mode="Markdown")
...
async def receive_py(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not fname.endswith(".py"):
        return await update.message.reply_text("❌ Needs .py")

    owner = get_owner(fname)
    if os.path.exists(os.path.join(UPLOAD_DIR, fname)) and owner and owner != uid and uid != ADMIN_ID:
        await update.message.reply_text(f"❌ **Taken!** `{fname}` is owned by another user.", parse_mode="Markdown")
        return WAIT_PY
    ...
    await update.message.reply_text("✅ Saved. Add extras?", reply_markup=extras_keyboard())
    return WAIT_EXTRAS
...
async def receive_extra_files(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if wait == 'req' and fname.endswith('.txt'):
        ...
        # For file uploads: stored as scripts/<scriptname>.env
        if context.user_data['type'] == 'file':
            path = os.path.join(work_dir, f"{target_id}.env")
        else:
            # For repo: stored as scripts/<repo>/.env
...
# =========================
# CONVERSATION 2: GIT CLONE + OPTIONAL .ENV BEFORE RUN
# =========================
WAIT_URL, WAIT_SELECT_FILE, WAIT_GIT_EXTRAS = range(2, 5)

@restricted
async def git_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("🌐 **Send Public Git Repository URL**", reply_markup=ReplyKeyboardMarkup([['🔙 Cancel']], resize_keyboard=True), parse_mode="Markdown")
    return WAIT_URL
...
async def receive_git_url(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await msg.edit_text("✅ **Cloned Successfully!**", parse_mode="Markdown")
        ...
        if os.path.exists(req_path):
            await update.message.reply_text("📦 Installing `requirements.txt`...", parse_mode="Markdown")
        ...
        await update.message.reply_text("👇 **Select Main File:**", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="Markdown")
...
async def select_git_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ✅ NEW: ask extras (optional .env) before run
    await query.edit_message_text(f"✅ Selected `{filename}`\n\nAdd extras (optional) then RUN.", parse_mode="Markdown")
    await query.message.reply_text("Extras?", reply_markup=extras_keyboard())
    return WAIT_GIT_EXTRAS

async def git_extras_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # Same extras logic, but for repo
...
async def git_extra_files_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # Same file receiver, but for repo
    return await receive_extra_files(update, context)

# =========================
# CONVERSATION 3: DEPLOY LINK GENERATOR (GITHUB → RENDER)
# =========================
WAIT_DEPLOY_REPO = 10
...
async def deploy_render_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "🚀 **Deploy to Render (GitHub Repo)**\n\n"
        "Send your **public GitHub repo URL**.\n"
        "Optional: add branch like this:\n"
...
def _parse_repo_and_branch(text: str):
    # Supports: https://github.com/u/r#branch=main
    branch = None
    repo = text.strip()

    if "#branch=" in repo:
        repo, frag = repo.split("#branch=", 1)
        branch = frag.strip() or None
...
async def deploy_render_receive(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not raw.startswith("http"):
        return await update.message.reply_text("❌ Invalid URL. Send a full https:// link.")

    repo, branch = _parse_repo_and_branch(raw)

    # Render deploy link: https://render.com/deploy?repo=<repo_url> (branch via /tree/<branch>) 2
    if branch:
        repo_for_render = f"{repo}/tree/{branch}"
    else:
        repo_for_render = repo

    deploy_url = f"https://render.com/deploy?repo={quote_plus(repo_for_render)}"

    await update.message.reply_text(
        "✅ **Render Deploy Link Generated**\n\n"
        f"🔗 `{deploy_url}`\n\n"
        "⚠️ Note:\n"
        "• Best if your repo has a `render.yaml` (Blueprint).\n"
        "• Render API can’t create free-tier services directly, so this link is the clean method.",
...
async def execute_logic(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not target_id:
        await msg_func("❌ Missing target_id.", reply_markup=main_menu_keyboard())
        return ConversationHandler.END
    ...
    if "|" in target_id:
        ...
        script_path = file
        env_path = os.path.join(work_dir, ".env")  # repo env lives here
    else:
        work_dir = UPLOAD_DIR
...
async def list_hosted(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("📂 **Your Apps:**", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="Markdown")
...
async def manage_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("man_"):
        ...
        if pid in running_processes:
            ...
            await query.edit_message_text(f"🛑 Stopped `{pid}`", parse_mode="Markdown")
        ...
        if "|" in pid:
            shutil.rmtree(os.path.join(UPLOAD_DIR, pid.split("|")[0]), ignore_errors=True)
        else:
